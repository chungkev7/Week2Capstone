...
public class Validator {
	...
	public static int getDeleteInt(Scanner scnr, String prompt) {
		// This approach use "hasNext" look ahead.
		System.out.print(prompt);
		while (!scnr.hasNextInt()) {
			scnr.nextLine(); // clear bad line.
	...
	public static int getTaskDone(Scanner scnr, String prompt) {
		// This approach use "hasNext" look ahead.
		System.out.print(prompt);
		while (!scnr.hasNextInt()) {
			scnr.nextLine(); // clear bad line.
	...
	public static int getInt(Scanner scnr, String prompt, int min, int max) {
		...
		do {
			...
			if (number < min) {
				isValid = false;
				System.out.println("The number must be from 1-5.");
			} else if (number > max) {
				isValid = false;
				System.out.println("The number must be from 1-5.");
			} else {
	...
	public static String getStringMatchingRegex(Scanner scnr, String prompt, String regex) {
		...
		do {
			...
			if (input.matches(regex)) {
				...
				System.out.println("Input must match the appropriate format with valid numbers (MM/DD/YYYY).");
				isValid = false;
			}
